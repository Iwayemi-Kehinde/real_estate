...
const Profile = () => {
  ...
  function handleFileUpload(file: any) {
    //get access to the storage from firebase
    const storage = getStorage(app);
    //get a unique name for each of the file
    const filename = new Date().getTime() + file.name;
    //where the image will save to.
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(
        `localhost:3000/api/user/delete/${currentUser._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      <form className="flex flex-col" onSubmit={handleSubmit}>
        ...
        <Link
          className="bg-green-700 text-white p-3 rounded-lg uppercase text-center hover:opacity-90 transition"
          to="/create-listing"
        >
          Create Listing
        </Link>
